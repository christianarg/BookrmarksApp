...
Object.defineProperty(exports, "__esModule", { value: true });
exports.TagsRoot = exports.TagsRootNew = exports.replaceTag = exports.filterTags = void 0;
...
var TagsRoot = /** @class */ (function (_super) {
    ...
    function TagsRoot() {
        ...
        _this.handleAddBookmark = function (tag, bookmarkModel) {
            var tags = _this.state.tags.slice();
            tag.bookmarks.push(bookmarkModel);
            tags = replaceTag(tags, tag);
            _this.setState({ tags: tags });
        };
        _this.handleAddOrEditTag = function (addOrEditTagResult, parentTag) {
            var tags = _this.state.tags.slice();
            if (parentTag) {
                ...
                if (addOrEditTagResult.oldName) {
                    // replace
                    parentTag.subTags = parentTag.subTags.map(function (x) { return x.name == addOrEditTagResult.oldName ? addOrEditTagResult : x; });
                }
                else {
                    // new
                    parentTag.subTags.push(addOrEditTagResult);
                }
                replaceTag(tags, __assign({}, parentTag));
            }
            else {
                // sin parent
                if (addOrEditTagResult.oldName) {
                    // replace
                    tags = tags.map(function (x) { return x.name == addOrEditTagResult.oldName ? addOrEditTagResult : x; });
                }
                else {
                    // new
        ...
        _this.handleEditBookmark = function (tag, editedBookmark) {
            var tags = _this.state.tags.slice();
            // replace bookmark
            tag.bookmarks = tag.bookmarks.map(function (x) { return x.name == editedBookmark.oldName ? editedBookmark : x; });
            tags = replaceTag(tags, tag);
            _this.setState({ tags: tags });
        };
        return _this;
    ...
    TagsRoot.prototype.componentDidMount = function () {
        this.setState({ tags: model_mobx_1.sampleBookrmarks.slice() });
    };
    TagsRoot.prototype.render = function () {
        ...
        if (this.state.tags) {
            return (React.createElement("div", null,
                ...
                React.createElement(Tags, { tags: this.state.tags, parentTag: null, onEditBookmark: this.handleEditBookmark, onAddBookmark: this.handleAddBookmark, onAddTag: this.handleAddOrEditTag }),
                React.createElement(AddOrEditTag, { isRoot: true, onAddOrEdit: function (newTag) { return _this.handleAddOrEditTag(newTag, null); } })));
        }
        return null;
    };
    return TagsRoot;
